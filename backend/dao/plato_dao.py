...
class PlatoDAO:
    ...
    def obtener_todos(self):
        ...
        cur = conn.cursor()
        cur.execute("SELECT * FROM plato ORDER BY id_plato;")
        filas = cur.fetchall()
        conn.close()
    ...
    def obtener_por_id(self, id_plato: int):
        conn = obtener_conexion()
        cur = conn.cursor()
        cur.execute("SELECT * FROM plato WHERE id_plato = %s;", (id_plato,))
        f = cur.fetchone()
        conn.close()
    ...
    def insertar(self, plato: Plato):
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO plato (nombre, precio, emoji) VALUES (%s, %s, %s) RETURNING id_plato;",
    ...
    def actualizar(self, id_plato: int, nombre=None, precio=None, emoji=None):
        ...
        cur = conn.cursor()
        cur.execute(
            "UPDATE plato SET nombre=%s, precio=%s, emoji=%s WHERE id_plato=%s "
